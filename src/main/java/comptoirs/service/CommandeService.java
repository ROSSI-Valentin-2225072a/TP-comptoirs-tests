...
public class CommandeService {
    ...
    public Ligne ajouterLigne(int commandeNum, int produitRef, @Positive int quantite) {
        ...
        if (produit.getUnitesEnStock() < quantite) {
            throw new IllegalStateException("La quantité commandée ne doit pas dépasser le stock disponible");
        }
        if (commande.getEnvoyeele() != null) {
            throw new IllegalStateException("On ne peut pas ajouter de ligne à une commande déjà livrée");
        }
    ...
    public Commande enregistreExpedition(int commandeNum) {
        var commande = commandeDao.findById(commandeNum).orElseThrow();
        if (commande.getEnvoyeele() != null) {
            throw new IllegalStateException("Cette commande a déjà été envoyée");
        }
